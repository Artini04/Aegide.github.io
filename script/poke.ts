...
class FusionHTMLCollection {
	hp: HTMLParagraphElement
	atk: HTMLParagraphElement
	def: HTMLParagraphElement
	spatk: HTMLParagraphElement
	spdef: HTMLParagraphElement
	spe: HTMLParagraphElement
	bs: HTMLParagraphElement
	ab: HTMLElement
	hab: HTMLElement
	...
	dexnum: HTMLParagraphElement
	fusionID: HTMLParagraphElement
	fusname: HTMLElement
...
// HTML ELEMENTS
const random1_btn = getByID('random1') as HTMLInputElement
const random2_btn = getByID('random2') as HTMLInputElement

const reset_btn = getByID('reset') as HTMLInputElement
const random_btn = getByID('random') as HTMLInputElement
const fuse_btn = getByID('fuse') as HTMLInputElement

const pkmn1_input = getByID('fname1') as HTMLInputElement
const pkmn2_input = getByID('fname2') as HTMLInputElement

const datalist = getByID('dlPkmn') as HTMLDataListElement
const dexnum1_span = getByID('dexnumber1') as HTMLSpanElement
const dexnum2_span = getByID('dexnumber2') as HTMLSpanElement
const fusionid1_span = getByID('fusionid1') as HTMLSpanElement
const fusionid2_span = getByID('fusionid2') as HTMLSpanElement

const fp1_div = getByID('FP1') as HTMLDivElement
const fp2_div = getByID('FP2') as HTMLDivElement

const pic1_img = getByID('pic1') as HTMLImageElement
const pic2_img = getByID('pic2') as HTMLImageElement

const hp1_e = getByID('hp1') as HTMLSpanElement
const atk1_e = getByID('atk1') as HTMLSpanElement
const def1_e = getByID('def1') as HTMLSpanElement
const spatk1_e = getByID('spatk1') as HTMLSpanElement
const spdef1_e = getByID('spdef1') as HTMLSpanElement
const spe1_e = getByID('spe1') as HTMLSpanElement
const ab1_e = getByID('ab1') as HTMLDivElement
const bs1_e = getByID('bs1') as HTMLSpanElement
const hab1_e = getByID('hab1') as HTMLDivElement

const weak14_e = getByID('weak14') as HTMLSpanElement
const weak12_e = getByID('weak12') as HTMLSpanElement
const weak11_e = getByID('weak11') as HTMLSpanElement
const weak105_e = getByID('weak105') as HTMLSpanElement
const weak1025_e = getByID('weak1025') as HTMLSpanElement
const weak100_e = getByID('weak100') as HTMLSpanElement

const hp2_e = getByID('hp2') as HTMLSpanElement
const atk2_e = getByID('atk2') as HTMLSpanElement
const def2_e = getByID('def2') as HTMLSpanElement
const spatk2_e = getByID('spatk2') as HTMLSpanElement
const spdef2_e = getByID('spdef2') as HTMLSpanElement
const spe2_e = getByID('spe2') as HTMLSpanElement
const ab2_e = getByID('ab2') as HTMLSpanElement
const bs2_e = getByID('bs2') as HTMLSpanElement
const hab2_e = getByID('hab2') as HTMLSpanElement

const weak24_e = getByID('weak24') as HTMLSpanElement
const weak22_e = getByID('weak22') as HTMLSpanElement
const weak21_e = getByID('weak21') as HTMLSpanElement
const weak205_e = getByID('weak205') as HTMLSpanElement
const weak2025_e = getByID('weak2025') as HTMLSpanElement
const weak200_e = getByID('weak200') as HTMLSpanElement

const type1_e = getByID('type1') as HTMLDivElement
const type2_e = getByID('type2') as HTMLDivElement

const fusion_HTMLCollection_1 = new FusionHTMLCollection(
	hp1_e,
	atk1_e,
	def1_e,
	spatk1_e,
...
const fusion_HTMLCollection_2 = new FusionHTMLCollection(
	hp2_e,
	atk2_e,
	def2_e,
	spatk2_e,
...
function createDatalistOptions() {
	for (const key in ids) {
		const option_element = document.createElement(
			'option'
...
function buildFusedPokemon(
	fused_pokemon_obj: FusedPokemon,
	head_pokemon: Pokemon,
	body_pokemon: Pokemon
): void {
	// Build FUSION DEX
	fused_pokemon_obj.id = Math.floor(
		head_pokemon.id + MAX_POKEMON * body_pokemon.id
	)
	...
	// Build FUSION TYPES
	const fusion_types: string[] = []

	if (head_pokemon.name === body_pokemon.name) {
		if (selfFusionTypeException.includes(head_pokemon.name)) {
			for (const key in selfFusionTypeFix[
				selfFusionTypeException.indexOf(head_pokemon.name)
			]) {
				fusion_types.push(
					selfFusionTypeFix[
						selfFusionTypeException.indexOf(
							head_pokemon.name
						)
					][key]
				)
		...
		const fused_types: string[] = fusType(
			head_pokemon.types,
			body_pokemon.types
		)
	...
	const type_effective_id: [number, number] = typeId(fusion_types)
	const type_effectiveness_array = []

	for (const key in typeName) {
		type_effectiveness_array.push(
			types[key][type_effective_id[0]] *
	...
	const abilities = fusionAbilities(
		head_pokemon.abilities,
		body_pokemon.abilities
	)
	const hidden_abilities = fusionHiddenAbilities(
		head_pokemon.abilities,
		body_pokemon.abilities,
		abilities
	)
 ...
 */
function buildUI(
	head_pokemon: FusedPokemon,
	body_pokemon: FusedPokemon,
	HTMLCollection: FusionHTMLCollection
): void {
	...
	// Build Image source
	appendSrcToImage(
		head_pokemon.fusion_id,
		HTMLCollection.image,
		HTMLCollection.fusionID
	)
	...
	// Build Weakness
	appendSrcToTypeEffectiveness(
		head_pokemon.type_effectiveness,
		HTMLCollection.weakness,
		'weakness-type'
	)
}

function appendSrcToType(
	list_array: string[],
	...
	for (const key in list_array) {
		const new_img = new Image() as HTMLImageElement
...
function appendSrcToTypeEffectiveness(
	list_array: number[],
	HTMLElements: FusionHTMLCollection['weakness'],
	class_name: string
) {
...
async function appendSrcToImage(
	fusionID: string,
	html_element: HTMLImageElement,
	elementFusionID: HTMLElement
): Promise<void> {
...
function getByID(htmlId: string): HTMLElement {
	return document.getElementById(htmlId)
}

function getCollByTag(htmlTag: string): HTMLCollectionOf<HTMLInputElement> {
...
function checkPokeName(name: string): string {
	if (nameFix.includes(name)) {
		name = nameException[nameFix.indexOf(name)]
...
async function fetchPokeAPI(name: string) {
...
function buildPokemon(response, pokemon_name: string, pokemon_obj: Pokemon) {
	// ID CHECK
	let is_valid_id: boolean = false
	pokemon_obj.name = response.name

	for (const key in ids) {
		if (ids[key][0].toString().toLowerCase() === pokemon_name) {
			is_valid_id = true
			pokemon_obj.id = ids[key][1] as number
		}
	}
...
function fusType(pokemon1: string[], pokemon2: string[]): string[] {
	let fusion_pokemon = []
...
function typeId(fusion_type): [number, number] {
...
function appendElementToList(
	...
	for (const key in list_array) {
		const created_span = document.createElement(
			'span'
		) as HTMLSpanElement
...
function fusionAbilities(pokemon_head_abilities, pokemon_body_abilities) {
	let rebuilt_body_abilities: string[] | string =
		pokemon_body_abilities[0]['ability']['name']
	let rebuilt_head_abilities: string[] | string
...
function fusionHiddenAbilities(
	pokemon_head_abilities,
	pokemon_body_abilities,
	fusion_abilities
): string[] {
	let rebuilded_head_abilities
	let rebuilded_body_abilities
	let all_abilities: string[] = []
	...
	let hiddenAbilities = all_abilities.filter(
		(n) => !fusion_abilities.includes(n)
	)
